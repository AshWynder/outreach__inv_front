...
import ProfessionalForm from './productForm.jsx';
...
export default function ProductState() {
  ...
  const isEditMode = !!id;
  const product = isEditMode ? products.find((product) => product._id === id) : null;
  console.log(product)
  ...
  }, [isEditMode, products.length, dispatch]);

  // if still loading
  if (isEditMode && (loading || products.length === 0)) {
    return (
      <Box
        sx={{
          display: 'flex',
