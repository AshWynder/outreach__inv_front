...
export default function ProfessionalForm() {
  ...
  const [formData, setFormData] = useState({
    name: '', description: '', costPrice: '', sellingPrice: '', vat: '',
    discountPercentage: '', quantityOnHand: '', quantityReserved: '', reorderLevel: '', reorderQuantity: '',
    unitOfMeasure: '', brand: '', model: '', color: '', weight: '',
    length: '', width: '', height: '', unit: '', processor: '',
    ram: '', storage: '', screenSize: '', category: '', supplier: '',
  });
        ...
        <Box component="form" onSubmit={handleSubmit}>
          ...
          <TabPanel value={activeTab} index={0}>
            <Grid container spacing={3} sx={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
              ...
              <Grid item xs={12} sm={6} sx={{ width: '60%' }}>
                <TextField fullWidth label="Description" name="description" value={formData.description} onChange={handleInputChange} required />
              </Grid>
              <Grid item xs={12} sm={6} sx={{ width: '60%' }}>
                <TextField fullWidth label="Cost Price" name="costPrice" value={formData.costPrice} onChange={handleInputChange} required />
              </Grid>
              <Grid item xs={12} sm={6} sx={{ width: '60%' }}>
                <TextField fullWidth label="Selling Price" name="sellingPrice" value={formData.sellingPrice} onChange={handleInputChange} required />
              </Grid>
              <Grid item xs={12} sm={6} sx={{ width: '60%' }}>
                <TextField fullWidth label="VAT" name="vat" value={formData.vat} onChange={handleInputChange} InputLabelProps={{ shrink: true }} />
              </Grid>
              <Grid item xs={12} sx={{ width: '60%' }}>
                <TextField fullWidth label="Discount in Percentage" name="discountPercentage" value={formData.discountPercentage} onChange={handleInputChange} />
              </Grid>
            </Grid>
          </TabPanel>
          ...
          <TabPanel value={activeTab} index={1}>
            <Grid container spacing={3} sx={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
              <Grid item xs={12} sx={{ width: '60%' }}>
                <TextField fullWidth label="Quantity On Hand" name="quantityOnHand" value={formData.quantityOnHand} onChange={handleInputChange} />
              </Grid>
              <Grid item xs={12} sm={6} sx={{ width: '60%' }}>
                <TextField fullWidth label="Quantity Reserved" name="quantityReserved" value={formData.quantityReserved} onChange={handleInputChange} />
              </Grid>
              <Grid item xs={12} sm={6} sx={{ width: '60%' }}>
                <TextField fullWidth label="Re-order Level" name="reorderLevel" value={formData.reorderLevel} onChange={handleInputChange} />
              </Grid>
              <Grid item xs={12} sm={6} sx={{ width: '60%' }}>
                <TextField fullWidth label="Re-order Quantity" name="reorderQuantity" value={formData.reorderQuantity} onChange={handleInputChange} />
              </Grid>
            </Grid>
          </TabPanel>
          ...
          <TabPanel value={activeTab} index={2} >
            <Grid container spacing={3} sx={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
              <Grid item xs={12} sm={6} sx={{ width: '60%' }}>
                <TextField fullWidth label="Length" name="length" value={formData.length} onChange={handleInputChange} type='number' />
              </Grid>
              <Grid item xs={12} sm={6} sx={{ width: '60%' }}>
                <TextField fullWidth label="Width" name="width" value={formData.width} onChange={handleInputChange} type='number'/>
              </Grid>
              <Grid item xs={12} sm={6} sx={{ width: '60%' }}>
                <TextField fullWidth label="Height" name="height" value={formData.height} onChange={handleInputChange} type='number'/>
              </Grid>
              <Grid item xs={12} sm={6} sx={{ width: '60%' }}>
                <Select fullWidth name="unit" value={formData.unit} onChange={handleInputChange} label="Unit of Measurement">
          ...
          <TabPanel value={activeTab} index={3}>
            <Grid container spacing={3} sx={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
              <Grid item xs={12} sm={6} sx={{ width: '60%' }}>
                <TextField fullWidth label="Processor Name" name="processor" value={formData.processor} onChange={handleInputChange} />
              </Grid>
              <Grid item xs={12} sm={6} sx={{ width: '60%' }}>
                <TextField fullWidth label="RAM type" name="ram" value={formData.ram} onChange={handleInputChange} />
              </Grid>
              <Grid item xs={12} sx={{ width: '60%' }}>
                <TextField fullWidth label="Storage type" name="storage" value={formData.storage} onChange={handleInputChange} />
              </Grid>
              <Grid item xs={12} sx={{ width: '60%' }}>
                <TextField fullWidth label="Screen Size" name="screenSize" value={formData.screenSize} onChange={handleInputChange} />
              </Grid>
            </Grid>
          </TabPanel>
          ...
          <Box sx={{ p: 3, display: 'flex', justifyContent: 'space-between', borderTop: '1px solid #e0e0e0' }}>
            <Box  sx={{bgcolor: '#1976d2', color: '#fff', p: 3, borderRadius: '1rem', height: '25px', display: 'flex', justifyContent: 'center', alignItems: 'center' }} onClick={() => setActiveTab(Math.max(0, activeTab - 1))} disabled={activeTab === 0}>
              Previous
            </Box>
            <Box sx={{ display: 'flex', gap: 2 }}>
              {activeTab < 3 ? (
                <Box sx={{bgcolor: '#1976d2', color: '#fff', p: 3, borderRadius: '1rem', height: '25px', display: 'flex', justifyContent: 'center', alignItems: 'center' }} onClick={() => setActiveTab(Math.min(4, activeTab + 1))}>
                  Next
                </Box>
              ) : (
                <Button variant="contained" color="success" type="submit" name='submit' sx={{borderRadius: '1rem'}}>
                  Submit Form
                </Button>
              )}
            </Box>
